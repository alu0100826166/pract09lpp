...
########################################################PRUEBAS PRACTICA 9
    ...
    context "Debe haber un modulo enumerable para clase lista" do
        it "Debe sacarse el minimo" do
            ...
            expect(@list.min).to eq(@docu)
       
        end
        
        it "Debe sacarse el maximo" do
            series = "Serie1"
            autores22 = Array.new
            autores22 = %w{ Autor1 Autor2 }
            isbns = { "isbn-1" => " 193778", "isbn-12" => " 978-19377" }
            @lib = Bibliografia::Libro.new(autores22, "Libro", series, "Editorial Libro", "Numero Edicion Libro", Date.new(2015, 11, 17), isbns)
                
            serie2s = "Serie2"
            autores11 = Array.new
            autores11 = %w{ Autor1 Autor2 }
            issns = "1133-9721"
            @rev = Bibliografia::Revista.new(autores11, "Revista", serie2s, "Editorial Revista", "Numero Edicion Revista", Date.new(2015, 11, 17), issns)
                
            autores33 = Array.new
            autores33 = %w{ Autor1 Autor2 }
            urls = "www.libroelectronico.com"
            @docu = Bibliografia::DocumentoElectronico.new(autores33, "DocElect", Date.new(2015, 11, 17), urls)
            
            @list=Bibliografia::ListaEnlazada.new(@lib)
            @list.insertar_delante(@rev)
            @list.insertar_detras(@docu)
            
            expect(@list.max).to eq(@rev)
        end
        
        it "Debe ordenar lista" do
            series = "Serie1"
            autores22 = Array.new
            autores22 = %w{ Autor1 Autor2 }
            isbns = { "isbn-1" => " 193778", "isbn-12" => " 978-19377" }
